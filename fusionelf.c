	...
	for(int i=0;i<elf2.headerElf.e_shnum;i++)
	{
		...
		if(elf2.tableSections[i].headerSec.sh_type==SHT_PROGBITS)
		{
			...
			for(int j=0;j<elf1.headerElf.e_shnum;j++)
			{
				...
				if(elf3.tableSections[j].headerSec.sh_type==SHT_PROGBITS && strcmp(elf2.tableSections[i].nomSec,elf3.tableSections[j].nomSec) == 0)
				{	//c'est que la section existe déjà, donc inutile de la copier dans elf3
					not_exist = 1;
				}
			}
